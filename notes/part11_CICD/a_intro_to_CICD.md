...
In [part 7](https://fullstackopen.com/en/part7/webpack) we had a look at [Webpack](https://webpack.js.org/) that is the current de facto tool for building a production version of a React or any other frontend JavaScript or TypeScript codebase.
...
Deploying a service (such as a web app) can vary in complexity. In [part 3](https://fullstackopen.com/en/part3/deploying_app_to_internet) our deployment workflow involved running some scripts manually and pushing the repository code to [Fly.io](https://fly.io/) or [Render](https://render.com/) hosting service.
...
Note that in this part we are assuming that the main branch (named *master* or *main*) contains the code that is running in production. There are numerous different [workflows](https://www.atlassian.com/git/tutorials/comparing-workflows) one can use with Git, e.g. in some cases, it may be a specific *release branch* that contains the code which is running in production.
